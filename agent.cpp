...
int main(int argc, char* argv[]) {
	...
	//sin_zero用??sockaddr_in?构填充到与struct sockaddr同?的?度，可以用bzero()或memset()函??其置?零
	...
	while (1) {
		...
		memset(&s_tmp, 0, sizeof(s_tmp));//把s_tmp所占存的東西先清空
		...
		if (segment_size > 0) {
			...
			//ipfrom儲存ip字串，利用strcmp判斷來源(當兩個數相等時會輸出0)
